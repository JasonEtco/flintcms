...
const fourOhFourHandler = async (req, res, template) => {
...
const noTemplateHandler = async (req, res, template) => {
  const compiled = await compile('server-missingTemplate', {request:req, template:template});
  return res.send(compiled);
}

const noHomepageHandler = async (req, res, template) => {
  const compiled = await compile('server-default-homepage', {request:req});
  return res.send(compiled);
}

const noHtmlHandler = async (req, res, template) => {
  const compiled = await compile('server-noHtml', {request:req, template:template});
  return res.send(compiled);
}
